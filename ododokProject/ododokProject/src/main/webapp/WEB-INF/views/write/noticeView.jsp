...
<div class = "bt">
	...
	<c:choose>
	 <c:when test = "${sessionScope.LoginDTO.f_id eq 'admin' }">
		<a href="./delete?w_number=${notice.w_number}" class="btn btn-light btn-sm abled" tabindex="-1" role="button" aria-disabled="true" style="background-color:pink">삭제</a>
		</c:when>
	</c:choose>
</div>
	...
	<table class="table table-dark table-striped">
		<tr>
			<td> 글번호: ${notice.w_number}</td>
			<td> 제목: ${notice.w_title}</td>
			<td> 작성자: ${notice.f_id}</td>
			<td> 작성일자: ${notice.w_date}</td>
			<td> 조회수: ${notice.w_hits}</td>
		</tr>	
	</table>
	 <img alt = "" src="/ododok/resources/upload/${notice.w_filename}">  
	 ${notice.w_contents}<br>
